...
def wait_for_update(timeout=1000):
	try:
		import xbmc
		...
		class Monitor(xbmc.Monitor):
			...
			def onNotification( self, sender, method, data):
				if sender == "xbmc": 
					log.debug(u'onNotification: {}'.format(method))

			def onScanFinished(self, library):
				log.debug('onScanFinished')
				log.debug(u'library: {}'.format(library))
				if library == 'video':
					self.do_exit = True
		...
		while not monitor.abortRequested() and count:
			for i in xrange(10):
				if monitor.waitForAbort(0.1) or monitor.do_exit:
					log.debug('Abort was requested while waiting. We should exit')
					del monitor
					return
			if count % 10 == 1:
				if not xbmc.getCondVisibility('Library.IsScanningVideo'):
					break
			count -= 1
			log.debug('Library Scanning Video - wait ({}s)'.format(timeout-count))
		del monitor

		"""
		while not xbmc.abortRequested and count: # and xbmc.getCondVisibility('Library.IsScanningVideo')
